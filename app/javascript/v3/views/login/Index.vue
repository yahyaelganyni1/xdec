    ...
    <section
      ...
      <div v-if="!email">
        ...
        <form class="space-y-5" @submit.prevent="submitLogin">
          <form-input v-model.trim="credentials.email" name="email_address" type="text" data-testid="email_input" required
            :label="$t('LOGIN.EMAIL.LABEL')" :placeholder="$t('LOGIN.EMAIL.PLACEHOLDER')"
            :has-error="$v.credentials.email.$error" @input="$v.credentials.email.$touch" />
          <form-input v-model.trim="credentials.password" type="password" name="password" data-testid="password_input"
            required :label="$t('LOGIN.PASSWORD.LABEL')" :placeholder="$t('LOGIN.PASSWORD.PLACEHOLDER')"
            :has-error="$v.credentials.password.$error" @input="$v.credentials.password.$touch">
          ...
          <submit-button :disabled="loginApi.showLoading" :button-text="$t('LOGIN.SUBMIT')"
